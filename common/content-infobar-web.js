...
;((globalThis) => {
  ...
  function isSingleFileComment(node) {
    return (
      ...
      node.textContent.includes(SINGLEFILE_COMMENT)
    )
  }
  ...
  function displayInfobar(infobarContent) {
    ...
    if (!SHADOW_DOM_SUPPORTED) {
      ...
      frameElement.style.setProperty(
        'width',
        infobarContent.getBoundingClientRect().width + 33 + 'px',
        'important'
      )
      frameElement.style.setProperty(
        'height',
        infobarContent.getBoundingClientRect().height + 21 + 'px',
        'important'
      )
    }
  }
  ...
  function createElement(tagName, parentElement) {
    ...
    Array.from(getComputedStyle(element)).forEach((property) =>
