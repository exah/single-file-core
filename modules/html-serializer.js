...
const OMITTED_START_TAGS = [
  {
    ...
    accept: (element) =>
      ...
      element.childNodes[0].nodeType == Node_ELEMENT_NODE,
  },
...
const OMITTED_END_TAGS = [
  {
    tagName: 'html',
    accept: (next) => !next || next.nodeType != Node_COMMENT_NODE,
  },
  {
    tagName: 'head',
    accept: (next) =>
      !next ||
      (next.nodeType != Node_COMMENT_NODE &&
        (next.nodeType != Node_TEXT_NODE ||
          !startsWithSpaceChar(next.textContent))),
  },
  {
    tagName: 'body',
    accept: (next) => !next || next.nodeType != Node_COMMENT_NODE,
  },
  {
    ...
    accept: (next, element) =>
      ...
      (next && ['LI'].includes(next.tagName)),
  },
  {
    tagName: 'dt',
    accept: (next) => !next || ['DT', 'DD'].includes(next.tagName),
  },
  {
    tagName: 'p',
    accept: (next) =>
  ...
  {
    tagName: 'rt',
    accept: (next) => !next || ['RT', 'RP'].includes(next.tagName),
  },
  {
    tagName: 'rp',
    accept: (next) => !next || ['RT', 'RP'].includes(next.tagName),
  ...
  {
    tagName: 'optgroup',
    accept: (next) => !next || ['OPTGROUP'].includes(next.tagName),
  },
  {
    tagName: 'option',
    accept: (next) => !next || ['OPTION', 'OPTGROUP'].includes(next.tagName),
  },
  {
    tagName: 'colgroup',
    accept: (next) =>
  ...
  {
    tagName: 'thead',
    accept: (next) => !next || ['TBODY', 'TFOOT'].includes(next.tagName),
  },
  {
    tagName: 'tbody',
    accept: (next) => !next || ['TBODY', 'TFOOT'].includes(next.tagName),
  ...
  {
    tagName: 'td',
    accept: (next) => !next || ['TD', 'TH'].includes(next.tagName),
  },
  {
    tagName: 'th',
    accept: (next) => !next || ['TD', 'TH'].includes(next.tagName),
...
function serializeElement(element, compressHTML, isSVG) {
  ...
  const omittedStartTag =
    compressHTML &&
    OMITTED_START_TAGS.find(
      (omittedStartTag) =>
        tagName == omittedStartTag.tagName && omittedStartTag.accept(element)
    )
  let content = ''
  if (!omittedStartTag || element.attributes.length) {
    ...
    Array.from(element.attributes).forEach(
      (attribute) =>
        (content += serializeAttribute(attribute, element, compressHTML))
    )
    content += '>'
  }
  ...
  const omittedEndTag =
    compressHTML &&
    OMITTED_END_TAGS.find(
      (omittedEndTag) =>
        tagName == omittedEndTag.tagName &&
        omittedEndTag.accept(element.nextSibling, element)
    )
...
function serializeAttribute(attribute, element, compressHTML) {
  ...
  if (!name.match(/["'>/=]/)) {
    ...
    const invalidUnquotedValue =
      !compressHTML || !value.match(/^[^ \t\n\f\r'"`=<>]+$/)
    content += ' '
