...
function process(doc, stylesheets, styles, options) {
  ...
  stylesheets.forEach((stylesheetInfo) => {
    ...
    if (cssRules) {
      ...
      docContent = getRulesTextContent(
        doc,
        cssRules,
        workStyleElement,
        docContent
  ...
  styles.forEach((declarations) => {
    ...
    docContent = getDeclarationsTextContent(
  ...
  if (globalThis.getComputedStyle && options.doc) {
    fontsInfo.used = fontsInfo.used.map((fontNames) =>
      fontNames.map((familyName) => {
        const matchedVar = familyName.match(/^var\((--.*)\)$/)
        if (matchedVar && matchedVar[1]) {
          const computedFamilyName = globalThis
            .getComputedStyle(options.doc.body)
            .getPropertyValue(matchedVar[1])
          return (
            (computedFamilyName &&
              computedFamilyName
                .split(',')
                .map((name) => helper.normalizeFontFamily(name))) ||
            familyName
          )
        }
        return familyName
    ...
    fontsInfo.used = fontsInfo.used.map((fontNames) =>
      helper.flatten(fontNames)
    )
  }
  const variableFound = fontsInfo.used.find((fontNames) =>
    fontNames.find((fontName) => fontName.startsWith('var(--'))
  )
  ...
  if (variableFound) {
    ...
    fontsInfo.used.forEach((fontNames) =>
      fontNames.forEach((familyName) => {
        if (
          fontsInfo.declared.find(
            (fontInfo) => fontInfo.fontFamily == familyName
          )
        ) {
          const optionalData =
            options.usedFonts &&
            options.usedFonts.filter((fontInfo) => fontInfo[0] == familyName)
    ...
    unusedFonts = fontsInfo.declared.filter(
      (fontInfo) => !filteredUsedFonts.has(fontInfo.fontFamily)
    )
  }
  stylesheets.forEach((stylesheetInfo) => {
    const cssRules = stylesheetInfo.stylesheet.children
  ...
  cssRules.forEach((ruleData) => {
    if (
      ...
      if (ruleData.type == 'Atrule' && ruleData.name == 'font-face') {
        const fontFamily = helper.normalizeFontFamily(
          getDeclarationValue(ruleData.block.children, 'font-family')
        )
        if (fontFamily) {
          const fontWeight =
            getDeclarationValue(ruleData.block.children, 'font-weight') || '400'
          const fontStyle =
            getDeclarationValue(ruleData.block.children, 'font-style') ||
            'normal'
          const fontVariant =
            getDeclarationValue(ruleData.block.children, 'font-variant') ||
            'normal'
          fontWeight
            .split(',')
            .forEach((weightValue) =>
              fontsInfo.declared.push({
                fontFamily,
                fontWeight: helper.getFontWeight(
                  helper.removeQuotes(weightValue)
                ),
                fontStyle,
                fontVariant,
              })
...
function filterUnusedFonts(
  ...
  for (let cssRule = cssRules.head; cssRule; cssRule = cssRule.next) {
    ...
    if (
      ...
      if (fontFamily) {
        const unicodeRange = getDeclarationValue(
          ruleData.block.children,
          'unicode-range'
        )
        if (
          unusedFonts.find((fontInfo) => fontInfo.fontFamily == fontFamily) ||
          !testUnicodeRange(docContent, unicodeRange) ||
      ...
      for (
        let declaration = ruleData.block.children.head;
        declaration;
      ...
      if (removedDeclarations.length) {
        removedDeclarations.forEach((removedDeclaration) =>
          ruleData.block.children.remove(removedDeclaration)
        )
      }
    }
  }
...
function testUsedFont(ruleData, familyName, declaredFonts, filteredUsedFonts) {
  let test
  const optionalUsedFonts =
    filteredUsedFonts && filteredUsedFonts.get(familyName)
  if (optionalUsedFonts && optionalUsedFonts.length) {
    let fontStyle =
      getDeclarationValue(ruleData.block.children, 'font-style') || 'normal'
    if (VALID_FONT_STYLES.find((rule) => fontStyle.trim().match(rule))) {
      const fontWeight = helper.getFontWeight(
        getDeclarationValue(ruleData.block.children, 'font-weight') || '400'
      )
      const declaredFontsWeights = declaredFonts
        .filter(
          (fontInfo) =>
            fontInfo.fontFamily == familyName && fontInfo.fontStyle == fontStyle
        )
        .map((fontInfo) => fontInfo.fontWeight)
        .sort(
          (weight1, weight2) =>
            Number.parseInt(weight1, 10) - Number.parseInt(weight2, 10)
        )
      let usedFontWeights = optionalUsedFonts.map((fontInfo) =>
        getUsedFontWeight(fontInfo, fontStyle, declaredFontsWeights)
      )
...
function testFontweight(fontWeight, usedFontWeights) {
  ...
  for (const value of fontWeight.split(/[ ,]/)) {
    test =
      test ||
      usedFontWeights.includes(helper.getFontWeight(helper.removeQuotes(value)))
  }
  return test
}
...
function getDeclarationValue(declarations, propertyName) {
  ...
  if (property) {
    try {
      return helper
        .removeQuotes(cssTree.generate(property.data.value))
        .toLowerCase()
    } catch (error) {
      // ignored
    }
...
function getFontFamilyNames(declarations) {
  let fontFamilyName = declarations.children.filter(
    (node) => node.property == 'font-family'
  ).tail
  let fontFamilyNames = []
  if (fontFamilyName) {
    let familyName = ''
  ...
  const font = declarations.children.filter(
    (node) => node.property == 'font'
  ).tail
  if (font && font.data && font.data.value) {
    try {
      const parsedFont = fontPropertyParser.parse(font.data.value)
      parsedFont.family.forEach((familyName) =>
        fontFamilyNames.push(helper.normalizeFontFamily(familyName))
      )
    } catch (error) {
      // ignored
    ...
    if (fontWeights.length > 1) {
      if (fontWeight >= 400 && fontWeight <= 500) {
        foundWeight = fontWeights.find(
          (weight) => weight >= fontWeight && weight <= 500
        )
        if (!foundWeight) {
      ...
      if (fontWeight < 400) {
        foundWeight = fontWeights
          .slice()
          .reverse()
          .find((weight) => weight <= fontWeight)
        if (!foundWeight) {
          foundWeight = findAscendingFontWeight(fontWeight, fontWeights)
        }
      }
  ...
  cssRules.forEach((ruleData) => {
    if (
      ...
      if (
        ...
        content = getDeclarationsTextContent(
          ruleData.block.children,
          workStylesheet,
          content
        )
...
function getDeclarationsTextContent(declarations, workStylesheet, content) {
  const contentText = getDeclarationUnescapedValue(
    declarations,
    'content',
    workStylesheet
  )
  const quotesText = getDeclarationUnescapedValue(
    declarations,
    'quotes',
    workStylesheet
  )
  ...
  if (unicodeRange) {
    ...
    const result = unicodeRanges.filter((rangeValue) => {
      ...
      if (range.length == 2) {
        ...
        regExpString =
          '[' + range[0] + '-' + transformRange('U+' + range[1]) + ']'
      }
      if (range.length == 1) {
        if (range[0].includes('?')) {
          ...
          regExpString =
            '[' +
            firstRange.replace(REGEXP_QUESTION_MARK, '0') +
            '-' +
            secondRange.replace(REGEXP_QUESTION_MARK, 'F') +
            ']'
        } else if (range[0]) {
