  ...
  (node) =>
    mergeElements(
      ...
      (node, previousSibling) =>
        ...
        node.media == previousSibling.media &&
        node.title == previousSibling.title
    ),
]
...
function process(doc, options) {
  ...
  const nodesWalker = doc.createTreeWalker(
    doc.documentElement,
    NodeFilter_SHOW_ALL,
    null,
    false
...
function mergeTextNodes(node) {
  if (node.nodeType == Node_TEXT_NODE) {
    if (
      node.previousSibling &&
      node.previousSibling.nodeType == Node_TEXT_NODE
    ) {
...
function mergeElements(node, tagName, acceptMerge) {
  if (
    node.nodeType == Node_ELEMENT_NODE &&
    ...
    while (
      previousSibling &&
      previousSibling.nodeType == Node_TEXT_NODE &&
    ...
    if (
      previousSibling &&
      previousSibling.nodeType == Node_ELEMENT_NODE &&
      previousSibling.tagName == node.tagName &&
      acceptMerge(node, previousSibling)
    ) {
...
function collapseWhitespace(node, options) {
  if (node.nodeType == Node_TEXT_NODE) {
    ...
    const spacePreserved =
      element.getAttribute(options.PRESERVED_SPACE_ELEMENT_ATTRIBUTE_NAME) == ''
    if (!spacePreserved) {
      ...
      if ((!element || noWhitespace) && textContent.length > 1) {
        node.textContent = textContent
          .replace(REGEXP_WHITESPACE, getWhiteSpace(node))
...
function noWhitespaceCollapse(element) {
  return (
    element &&
    !noWhitespaceCollapseElements.includes(element.tagName.toLowerCase())
  )
}

function removeComments(node) {
  if (
    node.nodeType == Node_COMMENT_NODE &&
  ...
  if (node.nodeType == Node_ELEMENT_NODE) {
    Array.from(node.attributes).forEach((attribute) => {
      if (safeToRemoveAttrs.includes(attribute.name.toLowerCase())) {
        ...
        if (
          attributeValue == '' ||
          (attributeValue || '').match(REGEXP_ENDS_WHITESPACE)
        ) {
...
function removeRedundantAttributes(node) {
  if (node.nodeType == Node_ELEMENT_NODE) {
    const tagRedundantAttributes =
      redundantAttributes[node.tagName.toLowerCase()]
    if (tagRedundantAttributes) {
      Object.keys(tagRedundantAttributes).forEach((redundantAttributeName) => {
        const tagRedundantAttributeValue =
          tagRedundantAttributes[redundantAttributeName]
        if (
          typeof tagRedundantAttributeValue == 'function'
            ? tagRedundantAttributeValue(node)
            : node.getAttribute(redundantAttributeName) ==
              tagRedundantAttributeValue
        ) {
