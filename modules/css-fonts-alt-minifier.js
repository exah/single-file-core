...
import { normalizeFontFamily, getFontWeight } from './../single-file-helper.js'
...
const REGEXP_URL_FUNCTION_WOFF_ALT =
  /^url\(\s*["']?data:application\/x-font-(woff)/
...
async function process(doc, stylesheets, fontDeclarations, fontTests) {
  ...
  stylesheets.forEach((stylesheetInfo) => {
    ...
    if (cssRules) {
      ...
      if (stylesheetInfo.mediaText && stylesheetInfo.mediaText != MEDIA_ALL) {
        ...
        fontsDetails.medias.set(
          'media-' + sheetIndex + '-' + stylesheetInfo.mediaText,
          mediaFontsDetails
        )
  ...
  await Promise.all(
    [...stylesheets].map(async ([, stylesheetInfo], sheetIndex) => {
      const cssRules = stylesheetInfo.stylesheet.children
      const media = stylesheetInfo.mediaText
      if (cssRules) {
        if (media && media != MEDIA_ALL) {
          await processFontFaceRules(
            cssRules,
            sheetIndex,
            fontsDetails.medias.get('media-' + sheetIndex + '-' + media),
            fontDeclarations,
            fontTests,
            stats
          )
...
function getFontsDetails(doc, cssRules, sheetIndex, mediaFontsDetails) {
  ...
  cssRules.forEach((ruleData) => {
    if (
      ...
      mediaFontsDetails.medias.set(
        'media-' + sheetIndex + '-' + mediaIndex + '-' + mediaText,
        fontsDetails
      )
      mediaIndex++
      ...
      const fontsDetails = createFontsDetailsInfo()
      mediaFontsDetails.supports.set(
        'supports-' + sheetIndex + '-' + supportsIndex + '-' + supportsText,
        fontsDetails
      )
      supportsIndex++
      getFontsDetails(doc, ruleData.block.children, sheetIndex, fontsDetails)
...
function processFontDetails(fontsDetails) {
  fontsDetails.fonts.forEach((fontInfo, fontKey) => {
    fontsDetails.fonts.set(
      fontKey,
      fontInfo.map((fontSource) => {
        ...
        if (fontFormatMatch && fontFormatMatch[1]) {
          fontFormat = fontFormatMatch[1]
            .replace(REGEXP_SIMPLE_QUOTES_STRING, '$1')
            .replace(REGEXP_DOUBLE_QUOTES_STRING, '$1')
            .toLowerCase()
        }
        if (!fontFormat) {
          ...
          if (fontFormatMatch && fontFormatMatch[1]) {
            ...
            const fontFormatMatch = fontSource.match(
              REGEXP_URL_FUNCTION_WOFF_ALT
            )
            if (fontFormatMatch && fontFormatMatch[1]) {
              fontFormat = fontFormatMatch[1]
            }
        ...
        return {
          src: fontSource.match(REGEXP_FONT_SRC)[1],
          fontUrl,
          format: fontFormat,
        }
      })
  ...
  fontsDetails.medias.forEach((mediaFontsDetails) =>
    processFontDetails(mediaFontsDetails)
  )
  fontsDetails.supports.forEach((supportsFontsDetails) =>
    processFontDetails(supportsFontsDetails)
  )
}

async function processFontFaceRules(
  cssRules,
  sheetIndex,
  fontsDetails,
  fontDeclarations,
  ...
  for (let cssRule = cssRules.head; cssRule; cssRule = cssRule.next) {
    ...
    if (
      ...
      if (fontInfo) {
        const processed = await processFontFaceRule(
          ruleData,
          fontInfo,
          fontDeclarations,
          fontTests,
...
async function processFontFaceRule(
  ruleData,
  fontInfo,
  fontDeclarations,
  fontTests,
  ...
  const srcDeclaration = ruleData.block.children.filter(
    (node) => node.property == 'src'
  ).tail
  if (srcDeclaration) {
    await Promise.all(
      fontInfo.map(async (source) => {
        if (fontTests.has(source.src)) {
          ...
          if (FontFace && source.fontUrl) {
            ...
            try {
              ...
              const urlNodes = cssTree.findAll(
                srcDeclaration.data,
                (node) => node.type == 'Url'
              )
              const declarationFontURLs = Array.from(fontDeclarations).find(
                ([node]) =>
                  urlNodes.includes(node) && node.value == source.fontUrl
              )
    ...
    const findSourceByFormat = (fontFormat, testValidity) =>
      fontInfo.find(
        (source) =>
          !source.src.match(EMPTY_URL_SOURCE) &&
          source.format == fontFormat &&
          (!testValidity || source.valid)
      )
    const filterSources = (fontSource) =>
      fontInfo.filter(
        (source) => source == fontSource || source.src.startsWith(LOCAL_SOURCE)
      )
    stats.fonts.processed += fontInfo.length
    stats.fonts.discarded += fontInfo.length
    const woffFontFound =
      findSourceByFormat('woff2-variations', true) ||
      findSourceByFormat('woff2', true) ||
      findSourceByFormat('woff', true)
    if (woffFontFound) {
      fontInfo = filterSources(woffFontFound)
    } else {
      const ttfFontFound =
        findSourceByFormat('truetype-variations', true) ||
        findSourceByFormat('truetype', true)
      if (ttfFontFound) {
        fontInfo = filterSources(ttfFontFound)
      } else {
        const otfFontFound =
          findSourceByFormat('opentype') ||
          findSourceByFormat('embedded-opentype')
        if (otfFontFound) {
          fontInfo = filterSources(otfFontFound)
        } else {
          fontInfo = fontInfo.filter(
            (source) =>
              (!source.src.match(EMPTY_URL_SOURCE) && source.valid) ||
              source.src.startsWith(LOCAL_SOURCE)
          )
        }
      }
    }
    ...
    fontInfo.reverse()
    try {
      srcDeclaration.data.value = cssTree.parse(
        fontInfo.map((fontSource) => fontSource.src).join(','),
        { context: 'value', parseCustomProperty: true }
      )
    } catch (error) {
      // ignored
    }
    ...
    property = ruleData.block.children.filter((node) => {
      try {
        return (
          ...
          !cssTree.generate(node.value).match(/\\9$/)
        )
      } catch (error) {
...
function getURL(urlFunction) {
  urlFunction = urlFunction.replace(
    /url\(-sf-url-original\\\(\\"(.*?)\\"\\\)\\ /g,
    ''
  )
  const urlMatch =
    urlFunction.match(REGEXP_URL_SIMPLE_QUOTES_FN) ||
