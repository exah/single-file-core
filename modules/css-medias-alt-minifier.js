...
function process(stylesheets) {
  ...
  stylesheets.forEach((stylesheetInfo, element) => {
    if (
      ...
      const removedRules = processRules(
        stylesheetInfo.stylesheet.children,
        stats
      )
...
function matchesMediaType(mediaText, mediaType) {
  const foundMediaTypes = helper.flatten(
    mediaQueryParser
      .parseMediaList(mediaText)
      .map((node) => getMediaTypes(node, mediaType))
  )
  return foundMediaTypes.find(
    (mediaTypeInfo) =>
      (!mediaTypeInfo.not &&
        (mediaTypeInfo.value == mediaType ||
          mediaTypeInfo.value == MEDIA_ALL)) ||
      (mediaTypeInfo.not &&
        (mediaTypeInfo.value == MEDIA_ALL || mediaTypeInfo.value != mediaType))
  )
}
    ...
    if (node.type == 'media-query') {
      ...
      if (node.type == 'media-type') {
        const nodeMediaType = {
          not: Boolean(
            indexNode &&
              parentNode.nodes[0].type == 'keyword' &&
              parentNode.nodes[0].value == 'not'
          ),
          value: node.value,
        }
        if (
          !mediaTypes.find(
            (mediaType) =>
              nodeMediaType.not == mediaType.not &&
              nodeMediaType.value == mediaType.value
          )
        ) {
