    ...
    if (!stylesheetInfo.scoped) {
      ...
      if (cssRules) {
        ...
        if (stylesheetInfo.mediaText && stylesheetInfo.mediaText != 'all') {
          mediaInfo = mediaAllInfo.medias.get(
    ...
    if (
      ...
      if (ruleData.type == 'Atrule' && ruleData.name == 'media') {
        ...
        processRules(
          ruleData.block.children,
          sheetIndex,
          mediaInfo.medias.get(
            'rule-' + sheetIndex + '-' + mediaRuleIndex + '-' + mediaText
          )
        )
        ...
        if (!ruleInfo && !pseudoSelectors) {
          ...
          if (
            !ruleData.prelude.children.size ||
            !ruleData.block.children.size
          ) {
            removedCssRules.push(cssRule)
          }
        }
      ...
      if (
        ...
        ruleData.type == 'Raw' ||
        (ruleData.type == 'Rule' &&
          (!ruleData.prelude || ruleData.prelude.type == 'Raw'))
      ) {
        removedCssRules.push(cssRule)
      }
    }
...
function processRuleInfo(ruleData, ruleInfo, pseudoSelectors) {
  ...
  for (
    let selector = ruleData.prelude.children.head;
    selector;
    ...
    if (
      !ruleInfo.matchedSelectors.has(selectorText) &&
      (!pseudoSelectors || !pseudoSelectors.has(selectorText))
    ) {
  ...
  if (!pseudoSelectorFound) {
    for (
      let declaration = ruleData.block.children.tail;
      declaration;
  ...
  removedDeclarations.forEach((declaration) =>
    ruleData.block.children.remove(declaration)
  )
...
function processStyleAttribute(styleData, mediaAllInfo) {
  ...
  if (styleInfo) {
    let propertyFound
    for (
      let declaration = styleData.children.head;
      declaration && !propertyFound;
    ...
    removedDeclarations.forEach((declaration) =>
      styleData.children.remove(declaration)
    )
  }
}
