  ...
  constructor(doc, stylesheets, styles) {
    ...
    stylesheets.forEach((stylesheetInfo) => {
      if (!stylesheetInfo.scoped) {
        ...
        if (cssRules) {
          if (
            stylesheetInfo.mediaText &&
            stylesheetInfo.mediaText != MEDIA_ALL
          ) {
            const mediaInfo = createMediaInfo(stylesheetInfo.mediaText)
            this.mediaAllInfo.medias.set(
              'style-' + sheetIndex + '-' + stylesheetInfo.mediaText,
              mediaInfo
            )
            getMatchedElementsRules(
              doc,
              cssRules,
              mediaInfo,
              sheetIndex,
...
function getMatchedElementsRules(
  doc,
  cssRules,
  mediaInfo,
  sheetIndex,
  ...
  if (DEBUG && cssRules.length > 1) {
    startTime = Date.now()
    log(
      '  -- STARTED getMatchedElementsRules',
      ' index =',
      sheetIndex,
      'rules.length =',
      cssRules.length
  ...
  cssRules.forEach((ruleData) => {
    if (
      ...
      if (ruleData.type == 'Atrule' && ruleData.name == 'media') {
        ...
        mediaInfo.medias.set(
          'rule-' + sheetIndex + '-' + mediaIndex + '-' + mediaText,
          ruleMediaInfo
        )
        mediaIndex++
        getMatchedElementsRules(
        ...
        const selectorsText = ruleData.prelude.children
          .toArray()
        ...
        if (!invalidSelector(selectorsText.join(','), workStylesheet)) {
          for (
            let selector = ruleData.prelude.children.head, selectorIndex = 0;
            selector;
            selector = selector.next, selectorIndex++
          ) {
            const selectorText = selectorsText[selectorIndex]
            const selectorInfo = { selector, selectorText, ruleInfo }
            getMatchedElementsSelector(
              doc,
              selectorInfo,
              styles,
              matchedElementsCache
...
function invalidSelector(selectorText, workStylesheet) {
  ...
  return workStylesheet.sheet
    ? !workStylesheet.sheet.cssRules.length
    : workStylesheet.sheet
}

function getMatchedElementsSelector(
  doc,
  selectorInfo,
  styles,
  matchedElementsCache
) {
  const filteredSelectorText = getFilteredSelector(
    selectorInfo.selector,
    selectorInfo.selectorText
  )
  const selectorText =
    filteredSelectorText != selectorInfo.selectorText
      ? filteredSelectorText
      : selectorInfo.selectorText
  ...
  if (!matchedElements) {
    try {
      ...
      if (selectorText != '.' + SINGLE_FILE_HIDDEN_CLASS_NAME) {
        matchedElements = Array.from(doc.querySelectorAll(selectorText)).filter(
          (matchedElement) =>
            !matchedElement.classList.contains(SINGLE_FILE_HIDDEN_CLASS_NAME) &&
            (matchedElement.style.getPropertyValue(DISPLAY_STYLE) != 'none' ||
              matchedElement.style.getPropertyPriority('display') !=
                'important')
        )
      }
    } catch (error) {
    ...
    if (matchedElements.length) {
      if (filteredSelectorText == selectorInfo.selectorText) {
        matchedElements.forEach((element) =>
          addRule(element, selectorInfo, styles)
        )
      } else {
        let pseudoSelectors = selectorInfo.ruleInfo.mediaInfo.pseudoRules.get(
          selectorInfo.ruleInfo.ruleData
        )
        if (!pseudoSelectors) {
          pseudoSelectors = new Set()
          selectorInfo.ruleInfo.mediaInfo.pseudoRules.set(
            selectorInfo.ruleInfo.ruleData,
            pseudoSelectors
          )
        }
...
function getFilteredSelector(selector, selectorText) {
  ...
  selector = {
    data: cssTree.parse(cssTree.generate(selector.data), {
      context: 'selector',
    }),
  }
  ...
  if (removedSelectors.length) {
    removedSelectors.forEach(({ parentSelector, selector }) => {
      if (
        parentSelector.data.children.size == 0 ||
        !selector.prev ||
        selector.prev.data.type == 'Combinator' ||
        selector.prev.data.type == 'WhiteSpace'
      ) {
        parentSelector.data.children.replace(
          selector,
          cssTree.parse('*', { context: 'selector' }).children.head
        )
      } else {
  ...
  function filterPseudoClasses(selector, parentSelector) {
    if (selector.data.children) {
      for (
        let childSelector = selector.data.children.head;
        childSelector;
        childSelector = childSelector.next
      ) {
    ...
    if (
      ...
      (selector.data.type == 'PseudoElementSelector' &&
        (testVendorPseudo(selector) ||
          IGNORED_PSEUDO_ELEMENTS.includes(selector.data.name)))
    ) {
...
function addRule(element, selectorInfo, styles) {
  ...
  if (!elementInfo) {
    ...
    if (elementStyle) {
      elementInfo.push({
        styleInfo: { styleData: elementStyle, declarations: new Set() },
      })
    }
...
function computeCascade(
  ...
  mediaInfo.elements.forEach((elementInfo /*, element*/) =>
    getDeclarationsInfo(elementInfo, workStylesheet /*, element*/).forEach(
      (declarationsInfo, property) => {
        if (
          declarationsInfo.selectorInfo.ruleInfo ||
          ...
          if (declarationsInfo.selectorInfo.ruleInfo) {
            ...
            const ascendantMedia =
              [mediaInfo, ...parentMediaInfo].find((media) =>
                media.rules.get(ruleData)
              ) || mediaInfo
            ascendantMedia.rules.set(ruleData, info)
            if (ruleData) {
              info.matchedSelectors.add(
                declarationsInfo.selectorInfo.selectorText
              )
            }
          } else {
  ...
  mediaInfo.medias.forEach((childMediaInfo) =>
    computeCascade(
      childMediaInfo,
      [mediaInfo, ...parentMediaInfo],
      mediaAllInfo,
      workStylesheet
    )
  )
...
function processDeclarations(
  ...
  for (
    let declaration = declarations.tail;
    declaration;
    ...
    if (
      ...
      if (
        !declarationInfo ||
        (declarationData.important && !declarationInfo.important)
      ) {
        declarationsInfo.set(declarationData, {
          selectorInfo,
          important: declarationData.important,
        })
...
function invalidDeclaration(declarationText, workStylesheet) {
  ...
  workStylesheet.textContent =
    'single-file-declaration { ' + declarationText + '}'
...
function sortRules(media) {
  media.elements.forEach((elementRules) =>
    elementRules.sort((ruleInfo1, ruleInfo2) =>
      ruleInfo1.styleInfo && !ruleInfo2.styleInfo
        ? -1
        : !ruleInfo1.styleInfo && ruleInfo2.styleInfo
        ? 1
...
function computeSpecificity(selector, specificity = { a: 0, b: 0, c: 0 }) {
  ...
  if (
    ...
    (selector.type == 'PseudoClassSelector' && selector.name != 'not')
  ) {
    specificity.b++
  }
  if (
    (selector.type == 'TypeSelector' && selector.name != '*') ||
    selector.type == 'PseudoElementSelector'
  ) {
    specificity.c++
  }
  if (selector.children) {
    selector.children.forEach((selector) =>
      computeSpecificity(selector, specificity)
    )
  }
  return specificity
}
