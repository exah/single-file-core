      ...
      if (message.method == INIT_REQUEST_MESSAGE) {
        if (event.source) {
          sendMessage(event.source, {
            method: ACK_INIT_REQUEST_MESSAGE,
...
function initRequestSync(message) {
  ...
  if (!TOP_WINDOW) {
    ...
    sendInitResponse({
      frames: [getFrameData(document, globalThis, windowId, message.options)],
      sessionId,
      requestedFrameId:
        document.documentElement.dataset.requestedFrameId && windowId,
    })
    if (message.options.userScriptEnabled && waitForUserScript) {
...
function initResponse(message) {
  message.frames.forEach((frameData) =>
    clearFrameTimeout('responseTimeouts', message.sessionId, frameData.windowId)
  )
  ...
  if (windowData) {
    ...
    message.frames.forEach((messageFrameData) => {
      let frameData = windowData.frames.find(
        (frameData) => messageFrameData.windowId == frameData.windowId
      )
      if (!frameData) {
    ...
    const remainingFrames = windowData.frames.filter(
      (frameData) => !frameData.processed
    ).length
    if (!remainingFrames) {
      windowData.frames = windowData.frames.sort(
        (frame1, frame2) =>
          frame2.windowId.split(WINDOW_ID_SEPARATOR).length -
          frame1.windowId.split(WINDOW_ID_SEPARATOR).length
      )
...
function processFramesAsync(
  ...
  sendInitResponse({
    ...
    sessionId,
    requestedFrameId:
      doc.documentElement.dataset.requestedFrameId && parentWindowId,
  })
  frameElements.forEach((frameElement, frameIndex) => {
    const windowId = parentWindowId + WINDOW_ID_SEPARATOR + frameIndex
    try {
      sendMessage(frameElement.contentWindow, {
        method: INIT_REQUEST_MESSAGE,
        windowId,
        sessionId,
        options,
      })
    ...
    requestTimeouts[windowId] = globalThis.setTimeout(
      () =>
        sendInitResponse({
          frames: [{ windowId, processed: true }],
          sessionId,
        }),
      TIMEOUT_INIT_REQUEST_MESSAGE
    )
...
function createFrameResponseTimeout(sessionId, windowId) {
  ...
  if (session && session.responseTimeouts) {
    session.responseTimeouts[windowId] = globalThis.setTimeout(
      () =>
        sendInitResponse({
          frames: [{ windowId: windowId, processed: true }],
          sessionId: sessionId,
        }),
      TIMEOUT_INIT_RESPONSE_MESSAGE
    )
  }
}
...
function cleanupFrames(frameElements, parentWindowId, sessionId) {
  frameElements.forEach((frameElement, frameIndex) => {
    ...
    try {
      sendMessage(frameElement.contentWindow, {
        method: CLEANUP_REQUEST_MESSAGE,
        windowId,
        sessionId,
      })
    } catch (error) {
...
function sendMessage(targetWindow, message, useChannel) {
  if (
    ...
    if (useChannel) {
      ...
      targetWindow.postMessage(
        MESSAGE_PREFIX + JSON.stringify(message),
        TARGET_ORIGIN
      )
    }
  }
  ...
  document.querySelectorAll(ALL_ELEMENTS_CSS_SELECTOR).forEach((element) => {
    ...
    if (shadowRoot) {
      frames = frames.concat(
        ...shadowRoot.querySelectorAll(FRAMES_CSS_SELECTOR)
      )
    }
  })
  return frames
