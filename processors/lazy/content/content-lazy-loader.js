  ...
  return new Promise(async (resolve) => {
    ...
    const observer = new MutationObserver(async (mutations) => {
      mutations = mutations.filter(
        (mutation) => mutation.type == ATTRIBUTES_MUTATION_TYPE
      )
      if (mutations.length) {
        const updated = mutations.filter((mutation) => {
          if (mutation.attributeName == 'src') {
            mutation.target.setAttribute(
              helper.LAZY_SRC_ATTRIBUTE_NAME,
              mutation.target.src
            )
          ...
          if (
            ...
            mutation.target.tagName == 'SOURCE'
          ) {
            return (
              !mutation.target.classList ||
              !mutation.target.classList.contains(
                helper.SINGLE_FILE_UI_ELEMENT_CLASS
              )
            )
          }
        })
...
async function deferLazyLoadEnd(observer, options, resolve) {
  await setAsyncTimeout(
    'loadTimeout',
    () => lazyLoadEnd(observer, options, resolve),
