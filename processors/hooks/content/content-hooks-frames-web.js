...
;((globalThis) => {
  const LOAD_DEFERRED_IMAGES_START_EVENT =
    'single-file-load-deferred-images-start'
  ...
  const LOAD_DEFERRED_IMAGES_RESET_EVENT =
  ...
  const warn =
    (console && console.warn && ((...args) => console.warn(...args))) ||
    (() => {})
  ...
  addEventListener(LOAD_DEFERRED_IMAGES_KEEP_ZOOM_LEVEL_START_EVENT, () =>
  ...
  function loadDeferredImagesStart(keepZoomLevel) {
    ...
    const scrollWidth = Math.max(
      scrollingElement.scrollWidth - clientWidth,
      clientWidth
    )
    ...
    if (!keepZoomLevel) {
      if (!globalThis._singleFile_getBoundingClientRect) {
        globalThis._singleFile_getBoundingClientRect =
          Element.prototype.getBoundingClientRect
        Element.prototype.getBoundingClientRect = function () {
          const boundingRect =
            globalThis._singleFile_getBoundingClientRect.call(this)
          if (this == scrollingElement) {
            boundingRect.__defineGetter__('height', () => scrollHeight)
            boundingRect.__defineGetter__(
              'bottom',
              () => scrollHeight + boundingRect.top
            )
            boundingRect.__defineGetter__('width', () => scrollWidth)
            boundingRect.__defineGetter__(
              'right',
              () => scrollWidth + boundingRect.left
            )
          }
          return boundingRect
        }
      ...
      globalThis.__defineGetter__('Image', function () {
        return function () {
          ...
          result.__defineSetter__('src', function (value) {
            image.src = value
            dispatchEvent(
              new CustomEvent(LOAD_IMAGE_EVENT, { detail: image.src })
            )
          })
          ...
          image.onload =
            ...
            image.onerror =
              (event) => {
                dispatchEvent(
                  new CustomEvent(IMAGE_LOADED_EVENT, { detail: image.src })
                )
    ...
    if (zoomFactor < 1) {
      ...
      const transformPriority =
        document.documentElement.style.getPropertyPriority('transform')
      const transformOrigin =
        document.documentElement.style.getPropertyValue('transform-origin')
      const transformOriginPriority =
        document.documentElement.style.getPropertyPriority('transform-origin')
      const minHeight =
        document.documentElement.style.getPropertyValue('min-height')
      const minHeightPriority =
        document.documentElement.style.getPropertyPriority('min-height')
    ...
    if (!keepZoomLevel) {
      ...
      if (window == window.top) {
        ;[...observers].forEach(([intersectionObserver, observer]) => {
          const getBoundingClientRectDefined =
            observer.options &&
            observer.options.root &&
            observer.options.root.getBoundingClientRect
          const rootBoundingRect =
            getBoundingClientRectDefined &&
            observer.options.root.getBoundingClientRect()
          ...
          if (targetElements) {
            const params = targetElements.map((target) => {
              ...
              const rootBounds = getBoundingClientRectDefined
                ? rootBoundingRect
                : docBoundingRect
              const time = 0
              return {
  ...
  addEventListener(LOAD_DEFERRED_IMAGES_KEEP_ZOOM_LEVEL_END_EVENT, () =>
  ...
  addEventListener(LOAD_DEFERRED_IMAGES_RESET_ZOOM_LEVEL_EVENT, () => {
    const transform =
      document.documentElement.style.getPropertyValue('-sf-transform')
    const transformPriority =
      document.documentElement.style.getPropertyPriority('-sf-transform')
    const transformOrigin = document.documentElement.style.getPropertyValue(
      '-sf-transform-origin'
    )
    const transformOriginPriority =
      document.documentElement.style.getPropertyPriority('-sf-transform-origin')
    const minHeight =
      document.documentElement.style.getPropertyValue('-sf-min-height')
    const minHeightPriority =
      document.documentElement.style.getPropertyPriority('-sf-min-height')
    document.documentElement.style.setProperty(
      'transform',
      transform,
      transformPriority
  ...
  function loadDeferredImagesEnd(keepZoomLevel) {
    document
      .querySelectorAll('[' + LAZY_LOAD_ATTRIBUTE + ']')
      .forEach((element) => {
  ...
  addEventListener(BLOCK_STORAGE_START_EVENT, () => {
    if (!globalThis._singleFile_localStorage) {
      ...
      globalThis.__defineGetter__('localStorage', () => {
    ...
    if (!globalThis._singleFile_indexedDB) {
      ...
      globalThis.__defineGetter__('indexedDB', () => {
  ...
  if (globalThis.FontFace) {
    ...
    document.fonts.delete = function (fontFace) {
      getDetailObject(fontFace.family).then((detail) =>
        dispatchEvent(new CustomEvent(DELETE_FONT_EVENT, { detail }))
      )
  ...
  if (globalThis.IntersectionObserver) {
    ...
    globalThis.IntersectionObserver = function () {
      ...
      const observeIntersection =
        IntersectionObserver.prototype.observe || intersectionObserver.observe
      ...
      if (unobserveIntersection) {
        intersectionObserver.unobserve = function (targetElement) {
          let targetElements = observedElements.get(intersectionObserver)
          if (targetElements) {
            targetElements = targetElements.filter(
    ...
    globalThis.IntersectionObserver.toString = function () {
