...
const DEFAULT_REPLACED_CHARACTERS = [
  '~',
  '+',
  '\\\\',
  '?',
...
function getInstance(utilOptions) {
  ...
  return {
    ...
    getValidFilename(
      filename,
      replacedCharacters = DEFAULT_REPLACED_CHARACTERS,
      replacementCharacter = DEFAULT_REPLACEMENT_CHARACTER
    ) {
      replacedCharacters.forEach(
        (replacedCharacter) =>
        ...
        reader.addEventListener(
          ...
          () => {
            if (content.startsWith(reader.result)) {
              ...
              this.truncateText(content, maxSize - 1)
                .then(resolve)
                .catch(reject)
            }
          },
    ...
    removeAlternativeFonts(doc, stylesheets, fontDeclarations, fontTests) {
      return modules.fontsAltMinifier.process(
        doc,
        stylesheets,
        fontDeclarations,
        fontTests
    ...
    PRESERVED_SPACE_ELEMENT_ATTRIBUTE_NAME:
      helper.PRESERVED_SPACE_ELEMENT_ATTRIBUTE_NAME,
  ...
  async function getContent(resourceURL, options) {
    ...
    if (
      options.blockMixedContent &&
      /^https:/i.test(options.baseURI) &&
    ...
    if (options.networkTimeout) {
      networkTimeoutPromise = new Promise((resolve, reject) => {
        ...
        networkTimeoutId = globalThis.setTimeout(
    ...
    try {
      const accept = options.acceptHeaders
        ? options.acceptHeaders[options.expectedType]
        : '*/*'
      if (options.frameId) {
        try {
          response = await Promise.race([
            fetchFrameResource(resourceURL, {
              ...
              referrer: options.resourceReferrer,
              headers: { accept },
            }),
            networkTimeoutPromise,
          ])
    ...
    if (options.asBinary) {
      ...
      try {
        ...
        if (
          ...
          buffer.byteLength > options.maxResourceSize * ONE_MB
        ) {
          return getFetchResponse(resourceURL, options)
        } else {
      ...
      if (
        response.status >= 400 ||
        (options.validateTextContentType &&
          contentType &&
          !contentType.startsWith(PREFIX_CONTENT_TYPE_TEXT))
      ) {
        return getFetchResponse(resourceURL, options)
      }
      if (!charset) {
...
function guessMIMEType(expectedType, buffer) {
  if (expectedType == 'image') {
    ...
    if (
      compareBytes([255, 255, 255, 255, 255, 255], [71, 73, 70, 56, 57, 97])
    ) {
      return 'image/gif'
    }
    if (
      compareBytes([255, 255, 255, 255, 255, 255], [71, 73, 70, 56, 59, 97])
    ) {
      return 'image/gif'
    }
    if (
  ...
  function compareBytes(mask, pattern) {
    ...
    if (buffer.byteLength >= pattern.length) {
      ...
      for (index = 0; index < mask.length && patternMatch; index++) {
        patternMatch =
          patternMatch && (value[index] & mask[index]) == pattern[index]
      }
      return patternMatch
    }
  }
