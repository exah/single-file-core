...
class Runner {
  constructor(options, root) {
    ...
    this.options = options
    this.options.url =
      this.options.url || (rootDocDefined && this.options.doc.location.href)
    const matchResourceReferrer = this.options.url.match(/^.*\//)
    this.options.resourceReferrer =
      this.options.passReferrerOnError &&
      matchResourceReferrer &&
      matchResourceReferrer[0]
    ...
    if (rootDocDefined) {
      const docData = util.preProcessDoc(
        this.options.doc,
        this.options.win,
        this.options
      )
    ...
    this.options.content =
      this.options.content ||
      (rootDocDefined ? util.serialize(this.options.doc) : null)
    this.onprogress = options.onprogress || (() => {})
  }

  async loadPage() {
    this.onprogress(
      new ProgressEvent(PAGE_LOADING, {
  ...
  async initialize() {
    this.onprogress(
      new ProgressEvent(RESOURCES_INITIALIZING, { pageURL: this.options.url })
    )
  ...
  async run() {
    if (this.root) {
      ...
      this.onprogress(
        new ProgressEvent(RESOURCES_INITIALIZED, {
          pageURL: this.options.url,
  ...
  getPageData() {
    if (this.root) {
      this.onprogress(
        new ProgressEvent(PAGE_ENDED, { pageURL: this.options.url })
      )
    }
  ...
  async executeStage(step) {
    ...
    this.onprogress(
      new ProgressEvent(STAGE_STARTED, {
        pageURL: this.options.url,
        step,
        frame,
      })
    ...
    STAGES[step].sequential.forEach((task) => {
      ...
      this.onprogress(
        new ProgressEvent(STAGE_TASK_STARTED, {
          pageURL: this.options.url,
          step,
          task: task.action,
          frame,
      ...
      this.onprogress(
        new ProgressEvent(STAGE_TASK_ENDED, {
          pageURL: this.options.url,
          step,
          task: task.action,
          frame,
    ...
    this.onprogress(
      new ProgressEvent(STAGE_ENDED, { pageURL: this.options.url, step, frame })
    )
    if (DEBUG) {
  ...
  executeTask(task) {
    if (
      !task.option ||
      (task.option.startsWith('!') && !this.options[task.option]) ||
      this.options[task.option]
    ) {
...
class BatchRequest {
  ...
  addURL(
    resourceURL,
    { asBinary, expectedType, groupDuplicates, baseURI, blockMixedContent } = {}
  ) {
        ...
        try {
          ...
          if (!this.cancelled) {
            resourceRequests.forEach((callbacks) => {
              ...
              const duplicate =
                duplicateCallbacks &&
                duplicateCallbacks.length > 1 &&
                duplicateCallbacks.includes(callbacks)
              callbacks.resolve({
                content: content.data,
                indexResource: currentIndexResource,
                duplicate,
              })
            })
          }
...
class Processor {
  ...
  initialize() {
    ...
    if (this.options.enableMaff) {
      this.maffMetaDataPromise = this.batchRequest.addURL(
        util.resolveURL('index.rdf', this.options.baseURI || this.options.url),
    ...
    if (
      ...
      this.options.frames.forEach(
        (frameData) => (this.maxResources += frameData.maxResources || 0)
      )
    }
  ...
  async loadPage(pageContent, charset) {
    ...
    if (this.options.saveRawPage) {
      let charset
      this.doc
        .querySelectorAll('meta[charset], meta[http-equiv="content-type"]')
        .forEach((element) => {
      ...
      if (
        charset &&
        content.charset &&
        charset.toLowerCase() != content.charset.toLowerCase()
      ) {
  ...
  async getPageData() {
    ...
    if (this.options.insertSingleFileComment) {
      ...
      if (
        firstComment.nodeType == 8 &&
        (firstComment.textContent.includes(util.COMMENT_HEADER_LEGACY) ||
          firstComment.textContent.includes(util.COMMENT_HEADER))
      ) {
      ...
      const infobarContent = (this.options.infobarContent || '')
        .replace(/\\n/g, '\n')
        .replace(/\\t/g, '\t')
      const commentNode = this.doc.createComment(
        '\n ' +
          (this.options.useLegacyCommentHeader
            ? util.COMMENT_HEADER_LEGACY
            : util.COMMENT_HEADER) +
          ' \n url: ' +
          infobarURL +
          ' \n saved date: ' +
      ...
      this.doc.documentElement.insertBefore(
        commentNode,
        this.doc.documentElement.firstChild
      )
    }
    if (
      this.options.insertCanonicalLink &&
      this.options.saveUrl.match(HTTP_URI_PREFIX)
    ) {
    ...
    if (this.options.insertMetaCSP) {
      ...
      metaTag.content =
        "default-src 'none'; font-src 'self' data:; img-src 'self' data:; style-src 'unsafe-inline'; media-src 'self' data:; script-src 'unsafe-inline' data:;"
      this.doc.head.appendChild(metaTag)
    }
    if (this.options.insertMetaNoIndex) {
      let metaElement = this.doc.querySelector(
    ...
    let filename =
      (await ProcessorHelper.evalTemplate(
        this.options.filenameTemplate,
        this.options,
        content
      )) || ''
    const replacementCharacter = this.options.filenameReplacementCharacter
    filename = util.getValidFilename(
      filename,
      this.options.filenameReplacedCharacters,
    ...
    if (
      !this.options.saveToGDrive &&
      !this.options.saveToGitHub &&
      !this.options.saveWithCompanion &&
      !this.options.saveWithWebDAV &&
      ((this.options.filenameMaxLengthUnit == 'bytes' &&
        util.getContentSize(filename) > this.options.filenameMaxLength) ||
        filename.length > this.options.filenameMaxLength)
    ) {
      const extensionMatch = filename.match(/(\.[^.]{3,4})$/)
      const extension =
        extensionMatch && extensionMatch[0] && extensionMatch[0].length > 1
          ? extensionMatch[0]
          : ''
      filename =
        this.options.filenameMaxLengthUnit == 'bytes'
          ? await util.truncateText(
              filename,
              this.options.filenameMaxLength - extension.length
            )
          : filename.substring(
              0,
              this.options.filenameMaxLength - extension.length
    ...
    const matchTitle =
      this.baseURI.match(/([^/]*)\/?(\.html?.*)$/) ||
      this.baseURI.match(/\/\/([^/]*)\/?$/)
    const pageData = {
      stats: this.stats.data,
      title:
        this.options.title ||
        (this.baseURI && matchTitle
          ? matchTitle[1]
          : url.hostname
          ? url.hostname
          : ''),
    ...
    if (this.options.retrieveLinks) {
      pageData.links = Array.from(
        new Set(
          Array.from(this.doc.links).map((linkElement) => linkElement.href)
        )
      )
    }
    return pageData
  ...
  preProcessPage() {
    if (this.options.win) {
      this.doc.body
        ...
        .forEach(
          (element) =>
            element instanceof this.options.win.HTMLElement &&
            this.doc.head.appendChild(element)
        )
    }
    if (this.options.images && !this.options.saveRawPage) {
      this.doc
        .querySelectorAll('img[' + util.IMAGE_ATTRIBUTE_NAME + ']')
        .forEach((imgElement) => {
          const attributeValue = imgElement.getAttribute(
            util.IMAGE_ATTRIBUTE_NAME
          )
          if (attributeValue) {
            ...
            if (imageData) {
              if (
                ...
                ((imageData.size &&
                  !imageData.size.pxWidth &&
                  !imageData.size.pxHeight) ||
                ...
                if (imageData.currentSrc) {
                  imgElement.dataset.singleFileOriginURL =
                ...
                if (this.options.loadDeferredImages) {
                  if (
                    (!imgElement.getAttribute('src') ||
                      imgElement.getAttribute('src') == util.EMPTY_RESOURCE) &&
  ...
  replaceStyleContents() {
    if (this.options.stylesheets) {
      this.doc.querySelectorAll('style').forEach((styleElement, styleIndex) => {
        const attributeValue = styleElement.getAttribute(
          util.STYLESHEET_ATTRIBUTE_NAME
        )
        if (attributeValue) {
  ...
  removeUnselectedElements() {
      ...
      Array.from(element.childNodes).forEach((node) => {
        if (node.nodeType == 1) {
          const isSelectedElement =
            node.getAttribute(util.SELECTED_CONTENT_ATTRIBUTE_NAME) == ''
    ...
    function removeNode(node) {
      if (
        (node.nodeType != 1 || !node.querySelector('svg,style,link')) &&
        canHideNode(node)
      ) {
        node.remove()
      } else {
  ...
  insertVideoLinks() {
    ...
    this.doc.querySelectorAll('video').forEach((videoElement) => {
      const attributeValue = videoElement.getAttribute(
        util.VIDEO_ATTRIBUTE_NAME
      )
      if (attributeValue) {
        ...
        if (videoElement && src) {
          ...
          const positionInlineParent =
            videoElement.parentNode.style.getPropertyValue('position')
          if (
            (!videoData.positionParent &&
              (!positionInlineParent || positionInlineParent != 'static')) ||
            videoData.positionParent == 'static'
          ) {
            videoElement.parentNode.style.setProperty(
              'position',
              'relative',
              'important'
            )
  ...
  removeFrames() {
    const frameElements = this.doc.querySelectorAll(
      'iframe, frame, object[type="text/html"][data]'
    )
    this.stats.set('discarded', 'frames', frameElements.length)
    this.stats.set('processed', 'frames', frameElements.length)
    this.doc
      .querySelectorAll('iframe, frame, object[type="text/html"][data]')
      .forEach((element) => element.remove())
  }

  removeEmbedScripts() {
    const JAVASCRIPT_URI_PREFIX = 'javascript:'
    this.onEventAttributeNames.forEach((attributeName) =>
      this.doc
    ...
    this.doc.querySelectorAll('[href]').forEach((element) => {
      if (
        element.href &&
        element.href.match &&
        element.href.trim().startsWith(JAVASCRIPT_URI_PREFIX)
      ) {
    ...
    const scriptElements = this.doc.querySelectorAll(
      'script:not([type="application/ld+json"]):not([' +
  ...
  removeDiscardedResources() {
    this.doc
      .querySelectorAll('.' + util.SINGLE_FILE_UI_ELEMENT_CLASS)
      .forEach((element) => element.remove())
    ...
    this.doc.querySelectorAll('noscript').forEach((noscriptElement) => {
      ...
      placeholderElement.innerHTML =
        noscriptElement.dataset.singleFileDisabledNoscript
      noscriptElement.replaceWith(placeholderElement)
      noscriptPlaceholders.set(placeholderElement, noscriptElement)
    })
    this.doc
      .querySelectorAll('meta[http-equiv=refresh], meta[disabled-http-equiv]')
      .forEach((element) => element.remove())
    Array.from(noscriptPlaceholders).forEach(
      ([placeholderElement, noscriptElement]) => {
        noscriptElement.dataset.singleFileDisabledNoscript =
          placeholderElement.innerHTML
    ...
    const replacedAttributeValue = this.doc.querySelectorAll(
    ...
    replacedAttributeValue.forEach((element) => {
      const relValue = element
        .getAttribute('rel')
        .replace(
          /(preconnect|prerender|dns-prefetch|preload|prefetch|manifest)/g,
          ''
        )
        .trim()
      if (relValue.length) {
    ...
    this.doc
      .querySelectorAll(
        'link[rel*=stylesheet][rel*=alternate][title],link[rel*=stylesheet]:not([href]),link[rel*=stylesheet][href=""]'
      )
      .forEach((element) => element.remove())
    ...
    this.doc
      ...
      .forEach((element) => element.removeAttribute('ping'))
    this.doc
      .querySelectorAll('link[rel=import][href]')
      .forEach((element) => element.remove())
  }

  replaceInvalidElements() {
    this.doc
      .querySelectorAll('template[' + util.INVALID_ELEMENT_ATTRIBUTE_NAME + ']')
      .forEach((templateElement) => {
        ...
        if (originalElement) {
          ...
          originalElement.childNodes.forEach((childNode) =>
            placeHolderElement.appendChild(childNode.cloneNode(true))
          )
        }
  ...
  setInputValues() {
    ...
    this.doc.querySelectorAll('select').forEach((select) => {
      select.querySelectorAll('option').forEach((option) => {
        const selected =
          option.getAttribute(util.INPUT_VALUE_ATTRIBUTE_NAME) != null
        if (selected) {
  ...
  replaceCanvasElements() {
    if (this.options.canvases) {
      this.doc.querySelectorAll('canvas').forEach((canvasElement) => {
        const attributeValue = canvasElement.getAttribute(
          util.CANVAS_ATTRIBUTE_NAME
        )
        if (attributeValue) {
          const canvasData = this.options.canvases[Number(attributeValue)]
          if (canvasData) {
            ProcessorHelper.setBackgroundImage(
              canvasElement,
              'url(' + canvasData.dataURI + ')'
            )
    ...
    if (this.options.fonts && this.options.fonts.length) {
      ...
      if (stylesheetContent) {
        ...
        if (existingStyleElement) {
          existingStyleElement.parentElement.insertBefore(
            styleElement,
  ...
  removeHiddenElements() {
    const hiddenElements = this.doc.querySelectorAll(
      '[' + util.HIDDEN_CONTENT_ATTRIBUTE_NAME + ']'
    )
    const removedElements = this.doc.querySelectorAll(
      '[' + util.REMOVED_CONTENT_ATTRIBUTE_NAME + ']'
    )
  ...
  resolveHrefs() {
    this.doc
      .querySelectorAll('a[href], area[href], link[href]')
      .forEach((element) => {
        ...
        if (!testIgnoredPath(href)) {
          let resolvedURL
          try {
            resolvedURL = util.resolveURL(
              href,
              this.options.baseURI || this.options.url
            )
          } catch (error) {
            // ignored
          }
          if (resolvedURL) {
            ...
            if (
              ...
              !resolvedURL.startsWith(url + '#!') &&
              !this.options.resolveFragmentIdentifierURLs
            ) {
        ...
        if (!videoElement.poster) {
          ...
          if (attributeValue) {
            ...
            if (src) {
              ...
              temporaryVideoElement.style.setProperty(
                'width',
                videoData.size.pxWidth + 'px',
                'important'
              )
              temporaryVideoElement.style.setProperty(
                'height',
                videoData.size.pxHeight + 'px',
                'important'
              )
              temporaryVideoElement.style.setProperty(
                'display',
              ...
              return new Promise((resolve) => {
                ...
                temporaryVideoElement.oncanplay = () => {
                  ...
                  try {
                    videoElement.poster = canvasElement.toDataURL(
                      'image/png',
                      ''
                    )
                  } catch (error) {
  ...
  resolveStyleAttributeURLs() {
    this.doc.querySelectorAll('[style]').forEach((element) => {
      if (this.options.blockStylesheets) {
        ...
        const declarationList = cssTree.parse(styleContent, {
          context: 'declarationList',
          parseCustomProperty: true,
        })
        ProcessorHelper.resolveStylesheetURLs(
          declarationList,
          this.baseURI,
          this.workStyleElement
        )
  ...
  async resolveStylesheetURLs() {
    await Promise.all(
      Array.from(this.doc.querySelectorAll('style, link[rel*=stylesheet]')).map(
        async (element) => {
          ...
          const scoped = Boolean(
            element.closest('[' + SHADOWROOT_ATTRIBUTE_NAME + ']')
          )
    ...
    if (this.options.rootDocument) {
      const newResources = Object.keys(this.options.updatedResources)
        .filter(
          (url) =>
            this.options.updatedResources[url].type == 'stylesheet' &&
            !this.options.updatedResources[url].retrieved
        )
    ...
    async function processElement(
      ...
      if (!options.blockStylesheets) {
        if (element.tagName == 'LINK') {
          stylesheet = await ProcessorHelper.resolveLinkStylesheetURLs(
            element.href,
            baseURI,
            options,
            workStyleElement
          ...
          stylesheet = cssTree.parse(element.textContent, {
            context: 'stylesheet',
            parseCustomProperty: true,
          })
          const importFound = await ProcessorHelper.resolveImportURLs(
            stylesheet,
            baseURI,
            options,
            workStyleElement
    ...
    if (!this.options.saveRawPage) {
      ...
      await Promise.all(
        frameElements.map(async (frameElement) => {
          ...
          const frameWindowId = frameElement.getAttribute(
            util.WIN_ID_ATTRIBUTE_NAME
          )
          if (this.options.frames && frameWindowId) {
            const frameData = this.options.frames.find(
              (frame) => frame.windowId == frameWindowId
            )
            if (frameData) {
  ...
  insertShadowRootContents() {
    ...
    if (options.shadowRoots && options.shadowRoots.length) {
      ...
      if (options.blockScripts) {
        this.doc
          .querySelectorAll('script[' + SCRIPT_TEMPLATE_SHADOW_ROOT + ']')
          .forEach((element) => element.remove())
      }
    ...
    function processElement(element) {
      const shadowRootElements = Array.from(
        element.querySelectorAll('[' + util.SHADOW_ROOT_ATTRIBUTE_NAME + ']')
      )
      shadowRootElements.forEach((element) => {
        const attributeValue = element.getAttribute(
          util.SHADOW_ROOT_ATTRIBUTE_NAME
        )
        if (attributeValue) {
          ...
          if (shadowRootData) {
            ...
            if (shadowDoc.head) {
              ...
              shadowDoc.head.childNodes.forEach((node) =>
                templateElement.appendChild(shadowDoc.importNode(node, true))
              )
            }
            if (shadowDoc.body) {
              shadowDoc.body.childNodes.forEach((node) =>
                templateElement.appendChild(shadowDoc.importNode(node, true))
              )
            }
  ...
  removeUnusedStyles() {
    if (!this.mediaAllInfo) {
      this.mediaAllInfo = util.getMediaAllInfo(
        this.doc,
        this.stylesheets,
        this.styles
      )
    }
    const stats = util.minifyCSSRules(
      this.stylesheets,
      this.styles,
      this.mediaAllInfo
    )
  ...
  async processStylesheets() {
    ...
    await Promise.all(
      [...this.stylesheets].map(([, stylesheetInfo]) =>
        ProcessorHelper.processStylesheet(
  ...
  async processPageResources() {
    ...
    let resourcePromises = processAttributeArgs.map(
      ([selector, attributeName, processDuplicates, removeElementIfMissing]) =>
    ...
    resourcePromises = resourcePromises.concat([
      ProcessorHelper.processXLinks(
        this.doc.querySelectorAll('use'),
        this.doc,
        this.baseURI,
        this.options,
        this.batchRequest
      ),
      ProcessorHelper.processSrcset(
        this.doc.querySelectorAll('img[srcset], source[srcset]'),
        this.baseURI,
        this.options,
        this.batchRequest
      ),
  ...
  async processScripts() {
    await Promise.all(
      Array.from(this.doc.querySelectorAll('script[src]')).map(
        async (element) => {
          let resourceURL
          let scriptSrc
          ...
          if (!this.options.blockScripts) {
            ...
            if (testValidURL(resourceURL)) {
              ...
              content.data = getUpdatedResourceContent(
                resourceURL,
                content,
                this.options
              )
              ...
              if (
                element.getAttribute('async') == 'async' ||
                element.getAttribute(util.ASYNC_SCRIPT_ATTRIBUTE_NAME) == ''
              ) {
  ...
  async removeAlternativeFonts() {
    await util.removeAlternativeFonts(
      this.doc,
      this.stylesheets,
      this.options.fontDeclarations,
      this.options.fontTests
    )
  }
      ...
      await Promise.all(
        frameElements.map(async (frameElement) => {
          ...
          if (frameWindowId) {
            ...
            if (frameData) {
              this.options.frames = this.options.frames.filter(
                (frame) => frame.windowId != frameWindowId
              )
              if (
                frameData.runner &&
                frameElement.getAttribute(util.HIDDEN_FRAME_ATTRIBUTE_NAME) !=
                  ''
              ) {
                ...
                if (
                  pageData.content.match(NOSCRIPT_TAG_FOUND) ||
                  pageData.content.match(CANVAS_TAG_FOUND) ||
                  pageData.content.match(SCRIPT_TAG_FOUND)
                ) {
                ...
                if (frameElement.tagName == 'OBJECT') {
                  frameElement.setAttribute(
                    'data',
                    'data:text/html,' + pageData.content
                  )
                } else {
  ...
  replaceStylesheets() {
    this.doc.querySelectorAll('style').forEach((styleElement) => {
      ...
      if (stylesheetInfo) {
        ...
        styleElement.textContent = generateStylesheetContent(
    ...
    this.doc.querySelectorAll('[style]').forEach((element) => {
      ...
      if (declarationList) {
        ...
        element.setAttribute(
          'style',
          generateStylesheetContent(declarationList, this.options)
        )
      } else {
  ...
  insertVariables() {
    if (this.cssVariables.size) {
      ...
      this.cssVariables.forEach(({ content, url }, indexResource) => {
        ...
        stylesheetContent += `${
          SINGLE_FILE_VARIABLE_NAME_PREFIX + indexResource
        }: `
  ...
  compressHTML() {
    ...
    util.minifyHTML(this.doc, {
      PRESERVED_SPACE_ELEMENT_ATTRIBUTE_NAME:
        util.PRESERVED_SPACE_ELEMENT_ATTRIBUTE_NAME,
    })
    if (this.options.displayStats) {
      this.stats.add(
        'discarded',
  ...
  cleanupPage() {
    ...
    if (metaCharset) {
      ...
      if (
        this.doc.head.querySelectorAll('*').length == 1 &&
        this.doc.body.childNodes.length == 0
      ) {
  ...
  resetZoomLevel() {
    const transform =
      this.doc.documentElement.style.getPropertyValue('-sf-transform')
    const transformPriority =
      this.doc.documentElement.style.getPropertyPriority('-sf-transform')
    const transformOrigin = this.doc.documentElement.style.getPropertyValue(
      '-sf-transform-origin'
    )
    const transformOriginPriority =
      this.doc.documentElement.style.getPropertyPriority('-sf-transform-origin')
    const minHeight =
      this.doc.documentElement.style.getPropertyValue('-sf-min-height')
    const minHeightPriority =
      this.doc.documentElement.style.getPropertyPriority('-sf-min-height')
  ...
  async insertMAFFMetaData() {
    ...
    if (maffMetaData && maffMetaData.content) {
      ...
      const originalURLElement = maffDoc.querySelector(
        'RDF > Description > originalurl'
      )
      const archiveTimeElement = maffDoc.querySelector(
        'RDF > Description > archivetime'
      )
      if (originalURLElement) {
        this.options.saveUrl = originalURLElement.getAttributeNS(
          NAMESPACE_RDF,
          'resource'
        )
      }
      if (archiveTimeElement) {
        const value = archiveTimeElement.getAttributeNS(
          NAMESPACE_RDF,
          'resource'
        )
        if (value) {
  ...
  async setDocInfo() {
    ...
    this.options.info = {
      description:
        descriptionElement && descriptionElement.content
          ? descriptionElement.content.trim()
          : '',
      lang: this.doc.documentElement.lang,
      author:
        authorElement && authorElement.content
          ? authorElement.content.trim()
          : '',
      creator:
        creatorElement && creatorElement.content
          ? creatorElement.content.trim()
          : '',
      publisher:
        publisherElement && publisherElement.content
          ? publisherElement.content.trim()
          : '',
      heading:
    ...
    this.options.infobarContent = await ProcessorHelper.evalTemplate(
      this.options.infobarTemplate,
      this.options,
      null,
      true
    )
...
class ProcessorHelper {
  static async evalTemplate(template = '', options, content, dontReplaceSlash) {
    ...
    template = await evalTemplateVariable(
      template,
      'page-title',
    ...
    template = await evalTemplateVariable(
      template,
      'page-heading',
      () => options.info.heading || 'No heading',
      dontReplaceSlash,
      options.filenameReplacementCharacter
    )
    template = await evalTemplateVariable(
    ...
    template = await evalTemplateVariable(
      template,
      'page-creator',
      () => options.info.creator || 'No creator',
      dontReplaceSlash,
      options.filenameReplacementCharacter
    )
    template = await evalTemplateVariable(
    ...
    template = await evalTemplateVariable(
      ...
      urlHref ? async () => util.digest('SHA-1', urlHref) : 'No href',
      dontReplaceSlash,
      options.filenameReplacementCharacter
    )
    template = await evalTemplateVariable(
    ...
    template = await evalTemplateVariable(
      template,
      'url-referrer',
      () => decode(options.referrer) || 'No referrer',
      dontReplaceSlash === undefined ? true : dontReplaceSlash,
      options.filenameReplacementCharacter
    )
    template = await evalTemplateVariable(
    ...
    template = await evalTemplateVariable(
      ...
      'url-pathname',
      () =>
        decode(url.pathname).replace(/^\//, '').replace(/\/$/, '') ||
        'No pathname',
      dontReplaceSlash === undefined ? true : dontReplaceSlash,
      options.filenameReplacementCharacter
    )
    template = await evalTemplateVariable(
      template,
      'url-pathname-flat',
      () => decode(url.pathname) || 'No pathname',
      false,
      options.filenameReplacementCharacter
    )
    template = await evalTemplateVariable(
    ...
    template = await evalTemplateVariable(
      template,
      'url-search',
      () => url.search.substring(1) || 'No search',
      dontReplaceSlash,
      options.filenameReplacementCharacter
    )
    ...
    template = await evalTemplateVariable(
      template,
      'tab-id',
      () => String(options.tabId || 'No tab id'),
      dontReplaceSlash,
      options.filenameReplacementCharacter
    )
    template = await evalTemplateVariable(
      template,
      'tab-index',
      () => String(options.tabIndex || 'No tab index'),
      dontReplaceSlash,
      options.filenameReplacementCharacter
    )
    template = await evalTemplateVariable(
      ...
      'url-last-segment',
      () =>
        decode(getLastSegment(url, options.filenameReplacementCharacter)) ||
        'No last segment',
      dontReplaceSlash,
      options.filenameReplacementCharacter
    )
    ...
    const bookmarkFolder =
      (options.bookmarkFolders && options.bookmarkFolders.join('/')) || ''
    template = await evalTemplateVariable(
      template,
      'bookmark-pathname',
      () => bookmarkFolder,
    ...
    async function evalDate(date, prefix = '') {
      if (date) {
        ...
        template = await evalTemplateVariable(
          template,
          prefix + 'time-iso',
          () => date.toISOString().split('T')[1].split('Z')[0],
          dontReplaceSlash,
          options.filenameReplacementCharacter
        )
        template = await evalTemplateVariable(
  ...
  static processShortcutIcons(doc) {
    let shortcutIcon = findShortcutIcon(
      Array.from(
        doc.querySelectorAll(
          'link[href][rel="icon"], link[href][rel="shortcut icon"]'
        )
      )
    )
    if (!shortcutIcon) {
      shortcutIcon = findShortcutIcon(
        Array.from(doc.querySelectorAll('link[href][rel*="icon"]'))
      )
      if (shortcutIcon) {
  ...
  static removeSingleLineCssComments(stylesheet) {
    const removedRules = []
    for (
      let cssRule = stylesheet.children.head;
      cssRule;
      cssRule = cssRule.next
    ) {
      const ruleData = cssRule.data
      if (
        ruleData.type == 'Raw' &&
        ruleData.value &&
        ruleData.value.trim().startsWith('//')
      ) {
  ...
  static async resolveImportURLs(
    ...
    await Promise.all(
      imports.map(async (node) => {
        const urlNode =
          cssTree.find(node, (node) => node.type == 'Url') ||
          cssTree.find(node, (node) => node.type == 'String')
        if (urlNode) {
          ...
          if (!testIgnoredPath(resourceURL) && testValidPath(resourceURL)) {
            ...
            if (
              testValidURL(resourceURL) &&
              !importedStyleSheets.has(resourceURL)
            ) {
              ...
              const mediaQueryListNode = cssTree.find(
                node,
                (node) => node.type == 'MediaQueryList'
              )
              if (mediaQueryListNode) {
                content.data = wrapMediaQuery(
                  content.data,
                  cssTree.generate(mediaQueryListNode)
                )
              }
              const importedStylesheet = cssTree.parse(content.data, {
                context: 'stylesheet',
                parseCustomProperty: true,
              })
    ...
    async function getStylesheetContent(resourceURL) {
      ...
      if (
        !(
          matchCharsetEquals(content.data, content.charset) ||
          matchCharsetEquals(content.data, options.charset)
        )
      ) {
        options = Object.assign({}, options, {
          charset: getCharset(content.data),
        })
  ...
  static resolveStylesheetURLs(stylesheet, baseURI, workStylesheet) {
    ...
    urls.map((urlNode) => {
      ...
      if (workStylesheet.sheet && workStylesheet.sheet.cssRules) {
        resourceURL = util.removeQuotes(
          workStylesheet.sheet.cssRules[0].style.getPropertyValue('content')
        )
      }
  ...
  static async resolveLinkStylesheetURLs(
    resourceURL,
    baseURI,
    options,
    workStylesheet
    ...
    if (
      ...
      if (
        ...
        return ProcessorHelper.resolveLinkStylesheetURLs(
          resourceURL,
          baseURI,
          options,
          workStylesheet
      ...
      let stylesheet = cssTree.parse(content.data, {
        context: 'stylesheet',
        parseCustomProperty: true,
      })
      const importFound = await ProcessorHelper.resolveImportURLs(
  ...
  static async processStylesheet(
    cssRules,
    baseURI,
    options,
    cssVariables,
    ...
    for (let cssRule = cssRules.head; cssRule; cssRule = cssRule.next) {
      ...
      if (ruleData.type == 'Atrule' && ruleData.name == 'charset') {
        ...
        if (ruleData.type == 'Rule') {
          ...
          (ruleData.name == 'media' || ruleData.name == 'supports')
        ) {
          promises.push(
          ...
          if (!options.blockFonts) {
            ...
            if (!testIgnoredPath(resourceURL) && testValidURL(resourceURL)) {
              ...
              if (!isDataURL(resourceURL) && options.saveOriginalURLs) {
                urlNode.value =
                  '-sf-url-original(' +
                  JSON.stringify(originalResourceURL) +
                  ') ' +
                  content
              } else {
    ...
    await Promise.all(
      urls.map(async (urlNode) => {
        ...
        if (!options.blockImages) {
          const resourceURL = normalizeURL(originalResourceURL)
          if (!testIgnoredPath(resourceURL) && testValidURL(resourceURL)) {
            let { content, indexResource, duplicate } =
              await batchRequest.addURL(resourceURL, {
                asBinary: true,
                expectedType: 'image',
                groupDuplicates: options.groupDuplicateImages,
              })
            if (!originalResourceURL.startsWith('#')) {
              const maxSizeDuplicateImages =
                options.maxSizeDuplicateImages || SINGLE_FILE_VARIABLE_MAX_SIZE
              if (
                duplicate &&
                options.groupDuplicateImages &&
                util.getContentSize(content) < maxSizeDuplicateImages
              ) {
                const varNode = cssTree.parse(
                  'var(' +
                    SINGLE_FILE_VARIABLE_NAME_PREFIX +
                    indexResource +
                    ')',
                  { context: 'value' }
                )
                ...
                cssVariables.set(indexResource, {
                  content,
        ...
        if (resourceURL != null) {
          ...
          if (options.saveOriginalURLs && !isDataURL(resourceURL)) {
            resourceElement.setAttribute(
              'data-sf-original-' + attributeName,
              resourceURL
            )
          }
            ...
            if (!testIgnoredPath(resourceURL)) {
              ...
              if (testValidPath(resourceURL)) {
                ...
                if (testValidURL(resourceURL)) {
                        ...
                        content = (
                          await util.getContent(resourceURL, {
                            ...
                            maxResourceSizeEnabled:
                              options.maxResourceSizeEnabled,
                            frameId: options.windowId,
                            resourceReferrer: options.resourceReferrer,
                            acceptHeaders: options.acceptHeaders,
                            networkTimeout: options.networkTimeout,
                  ...
                  if (
                    removeElementIfMissing &&
                    content == util.EMPTY_RESOURCE
                  ) {
                    resourceElement.remove()
                  } else if (content !== util.EMPTY_RESOURCE) {
                    const forbiddenPrefixFound =
                      PREFIXES_FORBIDDEN_DATA_URI.filter((prefixDataURI) =>
                        content.startsWith(prefixDataURI)
                      ).length
                    if (!forbiddenPrefixFound) {
                      const isSVG = content.startsWith(
                        PREFIX_DATA_URI_IMAGE_SVG
                      )
                      const maxSizeDuplicateImages =
                        options.maxSizeDuplicateImages ||
                        SINGLE_FILE_VARIABLE_MAX_SIZE
                      if (
                        expectedType == 'image' &&
                        processDuplicates &&
                        duplicate &&
                        ...
                        if (
                          ProcessorHelper.replaceImageSource(
                            resourceElement,
                            SINGLE_FILE_VARIABLE_NAME_PREFIX + indexResource,
                            options
                          )
                          ...
                          const declarationList = cssTree.parse(
                            resourceElement.getAttribute('style'),
                            {
                              context: 'declarationList',
                              parseCustomProperty: true,
                            }
      ...
      Array.from(resourceElements).map(async (resourceElement) => {
        ...
        if (!options.blockImages) {
          if (testValidPath(resourceURL) && !testIgnoredPath(resourceURL)) {
            ...
            if (testValidURL(resourceURL)) {
              ...
              if (originalResourceURL.startsWith(baseURI + '#')) {
                ...
                if (hashMatch && hashMatch[0]) {
                  ...
                  if (symbolElement) {
                    resourceElement.setAttribute(attributeName, hashMatch[0])
                    resourceElement.parentElement.insertBefore(
                      symbolElement,
                      resourceElement.parentElement.firstChild
                    )
                  }
                } else {
                  ...
                  resourceElement.setAttribute(
                    attributeName,
                    PREFIX_DATA_URI_IMAGE_SVG + ',' + content
                  )
                }
              }
            }
            ...
            resourceElement.setAttribute(
              attributeName,
              originalResourceURL.substring(resourceURL.length)
            )
          }
        } else {
          resourceElement.setAttribute(attributeName, util.EMPTY_RESOURCE)
  ...
  static replaceImageSource(imgElement, variableName, options) {
    ...
    if (attributeValue) {
      const imageData =
        options.images[
          Number(imgElement.getAttribute(util.IMAGE_ATTRIBUTE_NAME))
        ]
...
async function evalTemplateVariable(
  template,
  variableName,
  valueGetter,
  dontReplaceSlash,
  ...
  if (template) {
    const regExpVariable =
      '{\\s*' + variableName.replace(/\W|_/g, '[$&]') + '\\s*}'
...
function findShortcutIcon(shortcutIcons) {
  shortcutIcons = shortcutIcons.filter(
    (linkElement) => linkElement.href != util.EMPTY_RESOURCE
  )
  shortcutIcons.sort(
    (linkElement1, linkElement2) =>
      (parseInt(linkElement2.sizes, 10) || 16) -
      (parseInt(linkElement1.sizes, 10) || 16)
  )
...
function isDataURL(url) {
  return (
    url && (url.startsWith(DATA_URI_PREFIX) || url.startsWith(BLOB_URI_PREFIX))
  )
}
...
function testIgnoredPath(resourceURL) {
  return (
    resourceURL &&
    (resourceURL.startsWith(DATA_URI_PREFIX) || resourceURL == ABOUT_BLANK_URI)
  )
}
...
function testValidURL(resourceURL) {
  return (
    testValidPath(resourceURL) &&
    (resourceURL.match(HTTP_URI_PREFIX) ||
      resourceURL.match(FILE_URI_PREFIX) ||
      resourceURL.startsWith(BLOB_URI_PREFIX)) &&
...
class Stats {
  ...
  addAll(pageData) {
    if (this.options.displayStats) {
      Object.keys(this.data.discarded).forEach((key) =>
        this.add('discarded', key, pageData.stats.discarded[key] || 0)
      )
      Object.keys(this.data.processed).forEach((key) =>
        this.add('processed', key, pageData.stats.processed[key] || 0)
      )
    }
  }
}
