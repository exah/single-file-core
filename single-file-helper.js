...
const PRESERVED_SPACE_ELEMENT_ATTRIBUTE_NAME =
  'data-single-file-preserved-space-element'
...
function initUserScriptHandler() {
  addEventListener(
    ...
    () =>
      (globalThis._singleFile_waitForUserScript = async (eventPrefixName) => {
        const event = new CustomEvent(eventPrefixName + '-request', {
          cancelable: true,
        })
        const promiseResponse = new Promise((resolve) =>
          addEventListener(eventPrefixName + '-response', resolve)
        )
        dispatchEvent(event)
...
function preProcessDoc(doc, win, options) {
  doc
    .querySelectorAll(
      'noscript:not([' + DISABLED_NOSCRIPT_ATTRIBUTE_NAME + '])'
    )
    .forEach((element) => {
  ...
  if (doc.head) {
    doc.head
      .querySelectorAll(FLOW_ELEMENTS_SELECTOR)
      .forEach((element) => (element.hidden = true))
  }
  doc.querySelectorAll('svg foreignObject').forEach((element) => {
    const flowElements = element.querySelectorAll(
      'html > head > ' +
        FLOW_ELEMENTS_SELECTOR +
        ', html > body > ' +
        FLOW_ELEMENTS_SELECTOR
    )
...
function getElementsInfo(
  ...
  elements.forEach((element) => {
    ...
    if (
      !options.autoSaveExternalSave &&
      (options.removeHiddenElements ||
        options.removeUnusedFonts ||
      ...
      if (element instanceof win.HTMLElement) {
        if (options.removeHiddenElements) {
          elementKept =
            ((ascendantHidden || element.closest('html > head')) &&
              KEPT_TAG_NAMES.includes(element.tagName)) ||
            element.closest('details')
          if (!elementKept) {
            elementHidden =
              ascendantHidden || testHiddenElement(element, computedStyle)
            if (elementHidden) {
      ...
      if (!elementHidden) {
        ...
        if (options.removeUnusedFonts) {
          ...
          getUsedFont(
            win.getComputedStyle(element, ':first-letter'),
            options,
            data.usedFonts
          )
          getUsedFont(
            win.getComputedStyle(element, ':before'),
            options,
            data.usedFonts
          )
          getUsedFont(
            win.getComputedStyle(element, ':after'),
            options,
            data.usedFonts
          )
        }
    ...
    const shadowRoot =
      !(element instanceof win.SVGElement) && getShadowRoot(element)
    if (
      shadowRoot &&
      !element.classList.contains(SINGLE_FILE_UI_ELEMENT_CLASS)
    ) {
      ...
      try {
        if (
          ...
          shadowRootInfo.adoptedStyleSheets = Array.from(
            shadowRoot.adoptedStyleSheets
          ).map((stylesheet) =>
    ...
    if (
      ...
      if (
        elementKept ||
        element.getAttribute(KEPT_CONTENT_ATTRIBUTE_NAME) == ''
      ) {
  ...
  if (element.tagName == 'IMG') {
    const imageData = {
      currentSrc: elementHidden
        ...
        : (options.loadDeferredImages &&
            element.getAttribute(LAZY_SRC_ATTRIBUTE_NAME)) ||
          element.currentSrc,
    }
    ...
    if (computedStyle) {
      ...
      if (
        ...
        imageData.objectPosition =
          computedStyle.getPropertyValue('object-position')
      }
    }
  }
...
function getUsedFont(computedStyle, options, usedFonts) {
  if (computedStyle) {
    ...
    computedStyle
      ...
      .split(',')
      .forEach((fontFamilyName) => {
        fontFamilyName = normalizeFontFamily(fontFamilyName)
        if (
          !options.loadedFonts ||
          options.loadedFonts.find(
            (font) =>
              normalizeFontFamily(font.family) == fontFamilyName &&
          ...
          const fontVariant =
            computedStyle.getPropertyValue('font-variant') || 'normal'
          const value = [fontFamilyName, fontWeight, fontStyle, fontVariant]
          usedFonts.set(JSON.stringify(value), [
            fontFamilyName,
            fontWeight,
            fontStyle,
            fontVariant,
...
function postProcessDoc(doc, markedElements, invalidElements) {
  ...
  doc.querySelectorAll('meta[disabled-http-equiv]').forEach((element) => {
    element.setAttribute(
      'http-equiv',
      element.getAttribute('disabled-http-equiv')
    )
  ...
  if (!markedElements) {
    ...
    markedElements = doc.querySelectorAll(
      singleFileAttributes.map((name) => '[' + name + ']').join(',')
    )
  }
  ...
  if (invalidElements) {
    Array.from(invalidElements.entries()).forEach(
      ([element, placeholderElement]) => placeholderElement.replaceWith(element)
    )
  }
}
  ...
  if (doc) {
    ...
    doc.querySelectorAll('style').forEach((styleElement, styleIndex) => {
      try {
        ...
        if (
          !stylesheet ||
          stylesheet.cssRules.length != styleElement.sheet.cssRules.length
        ) {
          styleElement.setAttribute(STYLESHEET_ATTRIBUTE_NAME, styleIndex)
          contents[styleIndex] = Array.from(styleElement.sheet.cssRules)
            .map((cssRule) => cssRule.cssText)
            .join('\n')
        }
      } catch (error) {
...
function getSize(win, imageElement, computedStyle) {
  ...
  if (!pxWidth && !pxHeight) {
    ...
    if (computedStyle.getPropertyValue('box-sizing') == 'content-box') {
      ...
      const boxSizingPriority =
        imageElement.style.getPropertyPriority('box-sizing')
